...
% Delete duplicate point pair 1
% [~,i,~] = unique(cor1(:,1:2),'rows','first');
% cor1 = cor1(i,:); cor2 = cor2(i,:);
% [~,i,~] = unique(cor2(:,1:2),'rows','first');
% cor1 = cor1(i,:); cor2 = cor2(i,:);

if size(cor1,1)<20
    solution = []; rmse = []; cor1_new = []; cor2_new = []; return
end

[M,N] = size(cor1);
if strcmpi(change_form,'similarity')
    n = 2;
    max_iteration = M*(M-1)/2;
elseif strcmpi(change_form,'affine')
    n = 3;
    max_iteration = M*(M-1)*(M-2)/(2*3);
elseif strcmpi(change_form,'perspective')
    n = 4;
    max_iteration = M*(M-1)*(M-2)/(2*3);
end
...
% cor1_new = zeros(M,N);
% cor2_new = zeros(M,N);
cor1_new = cor1; cor2_new = cor2;

%%
% rand('seed',0);
for i = 1:iterations
%     if size(unique(cor1_new(:,1:2),'rows'),1)<20 || ...
%        size(unique(cor2_new(:,1:2),'rows'),1)<20
%         solution = []; rmse = []; cor1_new = []; cor2_new = []; return
%     end
    ...
    while(1)
        ...
        if(n==2 ... % similarity
            && (a(1)~=a(2)) ...
            && sum(cor11(1,1:2)~=cor11(2,1:2),2) && sum(cor22(1,1:2)~=cor22(2,1:2)))
            break;
        end
    ...
    if ~strcmpi(change_form,'perspective') % for similarity and affine 
